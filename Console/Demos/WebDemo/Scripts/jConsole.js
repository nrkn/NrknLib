var jConsole = {
  backgroundColor: '#000',
  foregroundColor: '#888',
  colors: [ '#000', '#008', '#080', '#088', '#800', '#808', '#880', '#bbb', '#888', '#00f', '#0f0', '#0ff', '#f00', '#f0f', '#ff0', '#fff' ],
  cellCache: [],
  ...
  blit: function( tiles ) {
    for( i in tiles ) {
      var tile = tiles[ i ];
      this.foregroundColor = tile.ForegroundColor;
      this.backgroundColor = tile.BackgroundColor;
      this.write( tile.Char );
    }
  },
  ...
  moveBufferArea: function( sourceLeft, sourceTop, sourceWidth, sourceHeight, targetLeft, targetTop ) {
    for( var y = sourceTop; y < sourceTop + sourceHeight; y++ ) {
      for( var x = sourceLeft; x < sourceLeft + sourceWidth; x++ ) {
        var cell = this.cellCache[ x ][ y ];
        var character = cell.character;
        var foregroundColor = cell.foregroundColor;
        var backgroundColor = cell.backgroundColor;
       
        this.setCell( ' ', x, y, this.foregroundColor, this.backgroundColor ); 
        this.setCell( character, targetLeft + ( x - sourceLeft ), targetTop + ( y - sourceTop ), foregroundColor, backgroundColor );
      }
    }
  },
