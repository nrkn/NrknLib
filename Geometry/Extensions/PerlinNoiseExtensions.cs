...
namespace NrknLib.Geometry.Extensions {
  public static class PerlinNoiseExtensions {
    // based heavily on http://www.gutgames.com/post/Perlin-Noise.aspx
    public static IGrid<byte> Generate( this IGrid<byte> grid, double frequency = 0.0525, double amplitude = 1.0, double persistance = 0.75, int octaves = 5 ) {
    ...
    public static double GetValue( this IGrid<double> noise, int x, int y, double frequency, double amplitude, double persistance, int octaves ) {
      ...
      return finalValue.Clamp( -1.0, 1.0 );
    }
